...
public class ReceptionServiceImpl implements ReceptionService {
    ...
    public ReceptionDTO getReception(Long receptionId) {
        ReceptionEntity entity = receptionRepository.findById(receptionId)
                .orElseThrow(() ->
                        new ReceptionNotFoundException("?묒닔 ID " + receptionId + "에 해당하는 데이터가 없습니다.")
                );
        return receptionResMapStruct.toDto(entity);
    }

    @Override
    public java.util.List<ReceptionDTO> getReceptionQueue(Long departmentId, Long doctorId, String date) {
    ...
    public void createReception(ReceptionDTO reception) {
        ...
        if (receptionRepository.existsByReceptionNo(reception.getReceptionNo())) {
            throw new IllegalArgumentException("?? ???? ???????: " + reception.getReceptionNo());
        }
    ...
    private Long resolveOrCreatePatientId(Long patientId, String patientName) {
        ...
        return patientRepository.findByPatientName(patientName.trim())
                ...
                .orElseGet(() -> {
                    kr.co.seoulit.reception.entity.PatientEntity entity = new kr.co.seoulit.reception.entity.PatientEntity();
    ...
    private String resolvePatientName(Long patientId) {
        return patientRepository.findById(patientId)
                ...
                .orElseThrow(() -> new ReceptionNotFoundException("?섏옄 ID " + patientId + "???????롫뮉 ?怨쀬뵠?怨? ??곷뮸??덈뼄."));
    }

    private String resolveDepartmentName(Long departmentId) {
        return departmentRepository.findById(departmentId)
                .map(d -> d.getDepartmentName())
                .orElseThrow(() -> new ReceptionNotFoundException("吏꾨즺怨?ID " + departmentId + "???????롫뮉 ?怨쀬뵠?怨? ??곷뮸??덈뼄."));
    }

    private String resolveDoctorName(Long doctorId) {
        return doctorRepository.findById(doctorId)
                .map(d -> d.getDoctorName())
                .orElseThrow(() -> new ReceptionNotFoundException("?섏궗 ID " + doctorId + "???????롫뮉 ?怨쀬뵠?怨? ??곷뮸??덈뼄."));
    }
}
